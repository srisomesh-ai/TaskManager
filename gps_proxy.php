...
if($action === 'update'){
    ...
    // Only send the dates when they are actually provided and non-empty. For a vehicle-to-vehicle
    // change the app omits these, so we must NOT send empty strings (that would blank the existing
    // installation/expiry dates on the server). Omitting them makes GPSWOX keep its current values.
    if(isset($_POST['installation_date']) && trim($_POST['installation_date'])!=='' && $_POST['installation_date']!=='0000-00-00'){
        $fields['installation_date'] = $_POST['installation_date'];
    }
    if(isset($_POST['expiration_date']) && trim($_POST['expiration_date'])!=='' && $_POST['expiration_date']!=='0000-00-00'){
