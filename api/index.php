...
case 'inv_save_item':
    ...
    $minStk  = intval($body['min_stock'] ?? 5);
    $notes   = trim($body['notes'] ?? '');
    if($iid){
        $pdo->prepare("UPDATE inventory_items SET name=?,category=?,model=?,unit=?,opening_bal=?,min_stock=?,notes=?,updated_at=NOW() WHERE id=?")
...
case 'inv_delete_item':
    if(!in_array($userRole,['admin','assigner'])){ http_response_code(403); echo json_encode(['error'=>'Not authorized']); break; }
    $iid = intval($body['id'] ?? 0);
    $hasMov = $pdo->prepare("SELECT COUNT(*) FROM inventory_movements WHERE item_id=?");
    $hasMov->execute([$iid]); 
    if($hasMov->fetchColumn() > 0){ echo json_encode(['error'=>'Cannot delete — item has movement history']); break; }
...
case 'inv_get_movements':
    ...
    $where = "1=1";
    $params = [];
    if(!empty($_GET['item_id'])){ $where .= " AND m.item_id=?"; $params[] = intval($_GET['item_id']); }
    if(!empty($_GET['type'])   ){ $where .= " AND m.type=?";    $params[] = $_GET['type']; }
    if(!empty($_GET['from'])   ){ $where .= " AND m.move_date>=?"; $params[] = $_GET['from']; }
    if(!empty($_GET['to'])     ){ $where .= " AND m.move_date<=?"; $params[] = $_GET['to']; }
    $stmt = $pdo->prepare("SELECT m.*, i.name as item_name, i.category, i.unit FROM inventory_movements m LEFT JOIN inventory_items i ON m.item_id=i.id WHERE $where ORDER BY m.created_at DESC LIMIT 500");
    $stmt->execute($params);
...
case 'inv_save_movement':
    ...
    $techName = trim($body['tech_name'] ?? '');
    $ref      = trim($body['ref_note'] ?? '');
    $date     = trim($body['move_date'] ?? date('Y-m-d'));
    if(!$itemId || !$type || $qty < 1){ echo json_encode(['error'=>'Item, type and quantity required']); break; }
    if(!in_array($type,['in','out','return','adjustment'])){ echo json_encode(['error'=>'Invalid type']); break; }
    if(($type==='out'||$type==='return') && !$techName){ echo json_encode(['error'=>'Technician required for issue/return']); break; }

    // Stock-out validation: check available office stock
    if($type === 'out'){
        $item = $pdo->prepare("SELECT opening_bal FROM inventory_items WHERE id=?");
        $item->execute([$itemId]); $itemRow = $item->fetch();
        $totalIn  = $pdo->prepare("SELECT COALESCE(SUM(qty),0) FROM inventory_movements WHERE item_id=? AND type='in'");
        $totalIn->execute([$itemId]); $tin = intval($totalIn->fetchColumn());
        $totalOut = $pdo->prepare("SELECT COALESCE(SUM(qty),0) FROM inventory_movements WHERE item_id=? AND type='out'");
        $totalOut->execute([$itemId]); $tout = intval($totalOut->fetchColumn());
        $returns  = $pdo->prepare("SELECT COALESCE(SUM(qty),0) FROM inventory_movements WHERE item_id=? AND type='return'");
        $returns->execute([$itemId]); $tret = intval($returns->fetchColumn());
        $officeStock = intval($itemRow['opening_bal'] ?? 0) + $tin - $tout + $tret;
        if($qty > $officeStock){
            echo json_encode(['error'=>'Only '.$officeStock.' in office stock — cannot issue '.$qty]);
            break;
        }
    }

    $pdo->prepare("INSERT INTO inventory_movements (item_id,type,qty,tech_name,ref_note,move_date,done_by) VALUES (?,?,?,?,?,?,?)")
        ->execute([$itemId,$type,$qty,$techName ?: null,$ref ?: null,$date,$cu['name']]);
    echo json_encode(['success'=>true,'id'=>intval($pdo->lastInsertId())]);
    break;
...
case 'inv_get_stock':
    ...
    $items = $pdo->query("SELECT i.*,
        ...
        COALESCE((SELECT SUM(qty) FROM inventory_movements WHERE item_id=i.id AND type='return'),0) as total_return,
        COALESCE((SELECT SUM(qty) FROM inventory_movements WHERE item_id=i.id AND type='adjustment'),0) as total_adj
        FROM inventory_items i ORDER BY i.category, i.name")->fetchAll();
    // Compute derived fields
    foreach($items as &$item){
        $item['office_stock'] = max(0, $item['opening_bal'] + $item['total_in'] - $item['total_out'] + $item['total_return'] + $item['total_adj']);
        $item['with_techs']   = max(0, $item['total_out'] - $item['total_return']);
        $item['closing_bal']  = $item['office_stock'];
    }
    unset($item);
    // Tech breakdown
    $techRows = $pdo->query("SELECT m.tech_name, m.item_id, m.type, SUM(m.qty) as qty FROM inventory_movements m WHERE m.tech_name IS NOT NULL GROUP BY m.tech_name, m.item_id, m.type")->fetchAll();
    $techStock = [];
    foreach($techRows as $row){
        $n = $row['tech_name']; $iid = $row['item_id'];
        if(!isset($techStock[$n])) $techStock[$n] = [];
        if(!isset($techStock[$n][$iid])) $techStock[$n][$iid] = 0;
        if($row['type']==='out')    $techStock[$n][$iid] += intval($row['qty']);
