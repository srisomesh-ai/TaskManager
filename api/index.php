...
// ════════════════════════════════════════════════════════
// INVENTORY API — admin and assigner only
// ════════════════════════════════════════════════════════
...
case 'inv_save_item':
    if(!in_array($userRole,['admin','assigner'])){ http_response_code(403); echo json_encode(['error'=>'Not authorized']); break; }
    $iid=$body['id']??0; $name=trim($body['name']??''); $cat=trim($body['category']??'');
    ...
    try {
        if($iid){
            $pdo->prepare("UPDATE inventory_items SET name=?,category=?,model=?,unit=?,opening_bal=?,min_stock=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?")
                ->execute([$name,$cat,$model,$unit,$opening,$minStk,$notes,intval($iid)]);
            echo json_encode(['success'=>true,'id'=>intval($iid)]);
        } else {
...
case 'inv_save_movement':
    if(!in_array($userRole,['admin','assigner'])){ http_response_code(403); echo json_encode(['error'=>'Not authorized']); break; }
    $itemId=intval($body['item_id']??0); $type=$body['type']??'';
    $qty=intval($body['qty']??0); $techName=trim($body['tech_name']??'');
    $ref=trim($body['ref_note']??''); $date=$body['move_date']??date('Y-m-d');
    ...
    if(($type==='out'||$type==='return')&&!$techName){ echo json_encode(['error'=>'Technician required']); break; }
    try {
        if($type==='out'){
            $r=$pdo->prepare("SELECT opening_bal, COALESCE((SELECT SUM(qty) FROM inventory_movements WHERE item_id=? AND move_type='in'),0) as ti, COALESCE((SELECT SUM(qty) FROM inventory_movements WHERE item_id=? AND move_type='out'),0) as to2, COALESCE((SELECT SUM(qty) FROM inventory_movements WHERE item_id=? AND move_type='return'),0) as tr FROM inventory_items WHERE id=?");
